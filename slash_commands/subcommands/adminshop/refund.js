...
async function refund(interaction, options) {
    ...
    if (!gameItem.state) return interaction.reply({
        embeds: [createError("La vente n'a pas encore **commencée** ! Utiliser `/adminshop delete <id>`")]
    });
    if (gameItem.state !== "done") return interaction.reply({
        embeds: [createError("La vente n'est pas encore **terminée** ! Utiliser `/adminshop cancel <id>`")]
    });
