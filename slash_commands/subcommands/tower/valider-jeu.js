...
function displayHealth(boss) {
  ...
  return `${filledEmoji.repeat(filledHP)}${hasIntermediate ? intermediateEmoji : ''}${emptyEmoji.repeat(emptyHP)}`;
}
      ...
      if (userDb.event.tower.etage % ETAGE_PAR_PALIER === 0) {
        return interaction.reply({
          embeds: [await createEmbed({
            title: `🏆 ${gameName} terminé !`,
            url: `https://store.steampowered.com/app/${appid}/`,
            desc: `En complétant **${gameName}**, ${author} arrive au **palier ${userDb.event.tower.etage / ETAGE_PAR_PALIER}** !
