...
const { leaveGroup, dissolveGroup, editMsgHubGroup } = require('../util/msg/group');
        ...
        for (let group of groups) {
            if (group.captain.userId === userDB.userId) {
                if(group.members.length === 1) {
                    ...
                    let memberGrp = group.members.find(u => u._id.equals(userDB._id));
                    let indexMember = group.members.indexOf(memberGrp);
                    group.members.splice(indexMember, 1);
                    group.size--;
                    const newCaptainDB = group.members[0];
                    const newCaptain = member.guild.members.cache.get(newCaptainDB.userId);
                    ...
                    logger.info(`${member.user.tag} a quitté le serveur, ${newCaptain.user.tag} est le nouveau capitaine du groupe : ${group.name}`);
                    const newMsgEmbed = new EmbedBuilder()
                        .setDescription(`${member.user.tag} a quitté le serveur, ${newCaptain.user.tag} est le nouveau capitaine du groupe ! (membres dans le groupe : ${group.size})`);
                    const channelGroup = await client.channels.cache.get(group.channelId);
