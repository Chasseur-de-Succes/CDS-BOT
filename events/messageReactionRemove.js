...
module.exports = {
	...
	async execute(msgReaction, user) {
        try {
            ...
            /* HALL HEROS / ZEROS */
            const idHeros = await msgReaction.client.getGuildChannel(msg.guildId, SALON.HALL_HEROS);
            const idZeros = await msgReaction.client.getGuildChannel(msg.guildId, SALON.HALL_ZEROS);
            ...
            if (hasPJ) {
                ...
                if (msg.attachments.every(m => m.contentType.startsWith('image'))) {
                    ...
                    if (isHallHeros) {
                        // maj nb reactions
                        await MsgHallHeros.findOneAndUpdate({msgId: msg.id}, {
                            $set: {
                    ...
                    if (isHallZeros) {
                        // maj nb reactions
                        await MsgHallZeros.findOneAndUpdate({msgId: msg.id}, {
                            $set: {
                                [`reactions.${emoji.name}`]: count
                            }
                        });
