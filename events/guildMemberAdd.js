...
module.exports = {
	...
	async execute(member) {
        ...
        // Nouveau 
        const nouveau = member.guild.roles.cache.find(r => r.name === 'Nouveau');
        if (nouveau)
            await member.roles.add(nouveau);

        // recup "a-lire"
        let alire = member.client.channels.cache.find(r => r.name === '💬a-lire');
        // recup "bienvenue"
...
> On espére te voir participer à la vie du groupe, et tu verras, ensemble, la chasse aux succès est encore plus amusante !`
            ...
            await member.client.channels.cache.get(bienvenue.id).send(msgBienvenue);
        }
